...
const EventSlider = ({ events, title }) => {
  ...
  return (
    ...
    >
            ...
            >
              ...
              >
                {event.portraitImage.startsWith("http") ? (
                  ...
                  <img
                    loading="lazy"
                    src={`${API_URL}/${event.portraitImage}`}
                    alt={`Movie ${event.title}`}
                    className="object-cover w-full h-auto mb-10 rounded-lg"
                  />
                )}

                <span className="px-4 py-[0.10rem] rounded-md bg-teal-500 text-white font-sm absolute top-1 left-1">
      ...
      {currentSlide < events.length - slidesToShow && (
        <button
          onClick={nextSlide}

...
const EventsSection = () => {
  ...
  const [city] = useState(localStorage.getItem("selectedCity") || "Kathmandu");
  ...
  const fetchEvents = useCallback(async () => {
    try {
      const response = await axios.get(`/api/v1/event/filtered-events`, {
        params: {
          page: currentPage,
          limit: 10,
          ...filters,
        ...
        <div className="min-h-[400px]">
          {filteredEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8">
              <p className="text-xl text-gray-600">No events found in {city}</p>
              {hasActiveFilters && (
                <button
                  onClick={clearFilters}
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {filteredEvents.map((event, index) => (
                ...
                >
                  {event.portraitImage.startsWith("http") ? (
                    <>
                      <img
                        loading="lazy"
                        src={event.portraitImage}
                        alt={event.title}
                    ...
                    <>
                      <img
                        loading="lazy"
                        src={`${API_URL}/${
                          event.portraitImage || "placeholder.png"
                        }`}
                        alt={event.title}
                        className="object-cover w-full h-48"
                      />

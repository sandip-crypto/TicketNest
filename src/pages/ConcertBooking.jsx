...
const ConcertBooking = ({ concert }) => {
  ...
  const [bookingDetails, setBookingDetails] = useState({
    ticketType: concert.ticketTypes[0]?.type || "",
    quantity: 1,
    name: `${user?.username}`,
    email: `${user?.email}`,
  ...
  const selectedTicketType = concert.ticketTypes.find(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "/api/v1/payment/initiate-concert-payment",
        {
